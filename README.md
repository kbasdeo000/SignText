...
We have created a web application which reads an image taken by the user and outputs the ASL letter they’re signing. Our final project is composedof a fully functional UI integrated with a machine learning model to translate ASL to text.
...
In the first terminal, go into the front-end project folder (React_Bootstrap) and run `npm start` to run the development server for the front-end. You will need to install the necessary dependencies and write a package.json file. 

In the second, run `python3 signtext.py` in the Back_End folder. This will run the backend server. 
